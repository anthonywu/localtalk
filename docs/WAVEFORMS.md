...
There are **two waveform implementations**, both using the same core technique (Unicode block characters + Rich `Live` display) but serving different purposes:
...
> **Note:** A playback waveform previously animated output levels during TTS, but its
> real-time terminal redraws caused audible crackling on the speakers, so it was removed.
> `AudioService.play_audio()` now plays without a live panel (it still polls an interrupt
> check and drains the device buffer).
